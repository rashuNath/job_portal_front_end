  ...
  //Check to see if the window is top if not then display button
    ...
    jQuery(window).scroll(function(){
      if (jQuery(this).scrollTop() > 300) {
        jQuery('.scrollToTop').addClass('transformed-top-50');
      } else {
        jQuery('.scrollToTop').removeClass('transformed-top-50');
      }
    });
    ...
    var $homeSlider = $('#mu-slider');
    $topHeaderHeight = $topHeader.height();
    $topHeader.waypoint(function (direction) {
        if(direction == 'down'){
            console.log('scrolling down');
            $topHeader.addClass('sticky');
            $homeSlider.css('margin-top',$topHeaderHeight+'px');
        }
    });
